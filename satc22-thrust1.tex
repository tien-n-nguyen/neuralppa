...
\subsubsection{Illustration Example for XAI}

\begin{wrapfigure}{l}{0.55\textwidth}
	\centering
	\includegraphics[width=3.4in]{pdg-2.png}
	\caption{Interpretation Sub-Graph for Figure~\ref{fig:motiv_1}}
	\label{fig:pdg}
\end{wrapfigure}

Let us illustrate the usage of our framework on a concrete example of
a real-world vulnerable code. Figure~\ref{fig:motiv_1} shows the
...
corresponding fix stated that {\em ``At line 6 and line 13, the driver
  ...
  \code{copy$\_$from$\_$user()}. The first fetched value (stored in
  \code{u$\_$cmd}) (line 6) is used to get the \code{in$\_$size} and
  \code{out$\_$size} elements and allocation a buffer
  (\code{s$\_$cmd}) at line 10 so as to copy the whole message to
  driver later~at~line 13, which means the copy size of the whole
  message (\code{s$\_$cmd}) is based on the old value
...
\begin{wrapfigure}{l}{0.65\textwidth}
  \centering
	...
	\lstset{
		...
		xleftmargin=3.1em,xrightmargin=2em, aboveskip=1em,
		framexleftmargin=0.5em,
		language=Java,
		basicstyle=\tiny\ttfamily,%scriptsize\ttfamily,
...
exit:
	...
	\end{lstlisting}
	\vspace{-0.16in}
	\caption{CVE-2016-6156 Vulnerability in Linux 4.6}
        \vspace{-0.06in}
	\label{fig:motiv_1}
%	\vspace{-5pt}
\end{wrapfigure}
